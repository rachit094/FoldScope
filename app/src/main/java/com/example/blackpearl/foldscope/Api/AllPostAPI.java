...
public class AllPostAPI extends AsyncTask<Void, Void, Integer> {

    public Context caller;
    public ResponseListener handler;
    public int result = -1;
    public String Tag = "AllPostAPI";
    private ArrayList<AllPostData> allPostDataArrayList;

    public AllPostAPI(Context context, ResponseListener responseListener) {
    ...
    protected Integer doInBackground(Void... arg0) {
        ...
        try {
            apiResponse = WebInterface.doGet(Const.URL_POST);
            result = apiResponse.code;
            if (apiResponse.code == 200) {
                if (apiResponse.response != null
                        && !apiResponse.response.equals("")) {
                    // this.Parse(apiResponse.response);
                    apiResponse = WebInterface.doGet(Const.URL_POST_ALL + apiResponse.header);
                    result = 0;
                } else {
                    result = -1;
                }
                ...
                Log.d(Tag, "All Post Json Response :----->" + apiResponse.response);
            }
    ...
    protected void onPostExecute(Integer result) {
        ...
        if (result == 0) {
            this.handler.onResponse(Const.URL_POST,
                    Const.API_RESULT.SUCCESS, allPostDataArrayList);
        } else {
            Toast.makeText(caller, R.string.server_error, Toast.LENGTH_LONG).show();
            this.handler.onResponse(Const.URL_POST,
                    Const.API_RESULT.FAIL, allPostDataArrayList);
        }
    }
    ...
    public void Parse(String response) {
        ...
        try {
            ...
            for (int j = 0; j < PostArray.length(); j++) {
                ...
                allPostData.setEvent_Id(PostArray.getJSONObject(j).getString("id"));
