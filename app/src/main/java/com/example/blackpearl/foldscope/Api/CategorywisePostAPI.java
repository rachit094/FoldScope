...
public class CategorywisePostAPI extends AsyncTask<Void, Void, Integer> {

    public Context caller;
    public ResponseListener handler;
    public int result = -1;
    private ArrayList<AllPostData> allPostDataArrayList = new ArrayList<AllPostData>();
    public String Category;
    public String Tag = "CategorywisePostAPI";

    public CategorywisePostAPI(Context context, ResponseListener responseListener, String Category) {
    ...
    protected Integer doInBackground(Void... arg0) {
        ...
        try {
            apiResponse = WebInterface.doGet(Const.URL_GETCATEGOIESWISEPOST +"page=1&per_page=5&categories="+ Category);
            result = apiResponse.code;
            if (apiResponse.code == 200) {
                if (apiResponse.response != null
                        && !apiResponse.response.equals("")) {
                    // this.Parse(apiResponse.response);
                    apiResponse = WebInterface.doGet(Const.URL_GETCATEGOIESWISEPOST + "page=1&per_page=" + apiResponse.header + "&categories=" + Category);
                    result = 0;
                } else {
                    result = -1;
                }
            ...
            Log.d(Tag, "Category wise Data Json Response :----->" + apiResponse.response);
    ...
    protected void onPostExecute(Integer result) {
        ...
        if (result == 0) {
            this.handler.onResponse(Const.URL_GETCATEGOIESWISEPOST, Const.API_RESULT.SUCCESS, allPostDataArrayList);
        } else {
            Toast.makeText(caller, R.string.server_error, Toast.LENGTH_LONG).show();
            this.handler.onResponse(Const.URL_GETCATEGOIESWISEPOST, Const.API_RESULT.FAIL, allPostDataArrayList);
        }
    }
